...
import { EDGES, STEPS, STEP_LEN, TOTAL_FRAMES, type StepIndex } from '../cards/constants';
import { ICON_NODES, ROUTES, nodeColor } from './constants';

/** ステップごとの系統色。エッジの色もこの並びから引く */
const PALETTE = ['#71d7ef', '#b398f9', '#b398f9', '#70dbb0', '#70dbb0', '#ffb17c'];

/** 実ロゴを出すチャネル。それ以外はピクトグラムのまま */
const BRAND_CHANNELS: Partial<Record<string, ChannelBrand>> = {
  resident: 'line',
  intake: 'line',
  reply: 'line',
  vendor: 'gmailLine',
...
/** 右側パネルに出すステップごとの説明 */
const STEP_NOTES = [
  '入居者から届いた会話と写真を受け付けます。',
  '問い合わせを物件・症状・緊急度に分解します。',
  'AIが優先度と初期対応方針を判定し、データへ書き戻します。',
  '業者マスタを参照して連絡。下書きの場合のみ承認を経て送信します。',
  'AIの方針をもとに、入居者へLINEで一次回答を返します。',
  '担当者が判定をレビューし、ルールと文脈を調整。次の問い合わせの判断に反映します。',
];
...
export const AgentFlowInquiryIcons: React.FC<{
  ...
  return (
    ...
    >
          ...
          >
            <span
              style={{
                ...
                display: 'grid',
                placeItems: 'center',
                background: i === step ? `${PALETTE[i]}20` : 'transparent',
              }}
            >
              {i + 1}
            </span>
      ...
      <div style={{position: 'absolute', right: 60, top: 48, color: '#a7b8cc', fontSize: 17}}>
        問い合わせを構造化し、判断の基準を人が育てる
      </div>
      ...
      <svg width="1920" height="1080" style={{position: 'absolute', inset: 0}}>
        ...
        {EDGES.map(edge => {
          ...
          // 業者マスタの参照線だけは系統色を持たせず、参照であることを灰色で示す
      ...
      <Tag x={nodeVariant ? 550 : 537} y={nodeVariant === 'actionRow' ? 480 : 514}>
        {nodeVariant ? '対応方針' : '初期対応方針'}
      </Tag>
      ...
      {ICON_NODES.map(node => {
        ...
        const title = `${node.num ? `${node.num}  ` : ''}${node.jp}`;
          ...
          >
            {brand ? (
              ...
              <>
                ...
                <IconNode
                  icon={<ServiceIcon name={node.icon} />}
                  service={title}
                  role={node.en}
                  variant={
                    ['policy', 'vendorDb', 'staff', 'resident', 'vendor'].includes(node.id)
                      ? 'circle'
                      : 'square'
                  }
                  color={color}
      ...
      {!hidePanel && <div
        style={{
          ...
          top: 580,
          width: 500,
          borderTop: '1px solid #334356',
          paddingTop: 24,
        }}
      >
        <div style={{fontSize: 20, color: PALETTE[step]}}>
          {String(step + 1).padStart(2, '0')} / {STEPS[step]}
        </div>
        <div style={{fontSize: 18, color: '#a7b8cc', lineHeight: 1.9, marginTop: 16}}>{STEP_NOTES[step]}</div>
        <div
          style={{
            display: 'flex',
            flexWrap: 'wrap',
