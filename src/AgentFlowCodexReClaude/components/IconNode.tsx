...
export const IconNode: React.FC<IconNodeProps> = ({
  icon, service, role, color = COLORS.blue, variant = "square", active = false, ports = [],
}) => {
  const card = variant === "card";
  const port = (side: "left" | "right" | "bottom"): React.CSSProperties => ({
    position: "absolute", width: 12, height: 12,
    background: active ? color : "#a5b2c4", border: `3px solid ${COLORS.bg}`,
  ...
  const labels = <div style={{textAlign: "center", padding: "0 8px", marginTop: card ? 22 : 18}}>
    ...
    {role && <div style={{fontSize: 17, lineHeight: 1.4, marginTop: 7, color: COLORS.muted, overflowWrap: "anywhere"}}>{role}</div>}
  </div>;
  return <div role="img" aria-label={`${service}${role ? `：${role}` : ""}${active ? "（アクティブ）" : ""}`} style={{width: 208, color: COLORS.text, fontFamily: '"Hiragino Sans", sans-serif'}}>
