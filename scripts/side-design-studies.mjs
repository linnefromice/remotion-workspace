...
try {
 ...
 if(!onlyVideo){
  ...
  for(const frame of [0,359,360,599,600,839])await still(newId,frame);
  ...
  await writeFile(path.join(out,'index.html'),template.replace('__NEW_STEPS__',strip(newId)).replace('__OLD_STEPS__',strip('ClaimIntake-DecisionStory-Side')).replace('__VARIANTS__',variants));
 }
 if(!onlyStills){
  await renderMedia({serveUrl,composition:all.find(c=>c.id===newId),codec:'h264',pixelFormat:'yuv420p',imageFormat:'png',outputLocation:path.join(out,`${newId}.mp4`),puppeteerInstance:browser,concurrency:4});
  console.log('Rendered',newId);
 }
