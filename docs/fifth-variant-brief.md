...
いま Transit が持っている形はこう（`design-studies/agent-diagram/model.ts`）。

- 主体を3本の帯（外部チャネル / エージェント / 人・参照情報）に分ける
- 帯の中で横に並べる
- 明るい地

これを、**題材の `constants.ts` が持っている `NODES[].tone` と `cx/cy`（またはそのグリッド）から
導けるか**を見る。`tone` は主体なので、帯の割り当てには使えるはず。
