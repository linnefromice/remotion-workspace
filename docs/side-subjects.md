...
`subjects/DemoScreens.tsx`で工程ごとの画面を表示する。Proposalは相談チャット・希望条件フォーム・候補JSON・除外一覧・復帰レビュー・提案カード。Inquiryは写真付き相談・受付フォーム・判定JSON・連絡下書き・LINE一次回答・レビュー。Restorationは立会い入力・分類JSON・区分照合・残存価値・見積・担当者確認・精算書。

判断値は既存の題材データを参照し、候補名や会話文は画面説明用の表示例として同ファイルにまとめている。Inquiryの連絡画面は下書き承認の経路を表示。Restorationでは残存価値と作業費を区別する。変更対象は3題材のDemoDiagramのみ。

外部向けの相談・回答・提案はブルー基調、社内向けの管理・立会い・AI JSONはブラック基調。画面上の外部／社内ラベルでも利用者を示す。太い端末枠や装飾イラストを使わず、アプリヘッダー、ナビゲーション、添付ファイル情報、フォーム、候補番号で画面を構成する。配色は共通の意味付きトークンで統一。
